            ...
            while iterator < limit:
                try:
                            ...
                            if not equal:
                                if size > dimension:
                                    if tokenized_answer.shape[1] > dimension and tokenized_question_size > dimension:
                                        ...
                                        anwser = tokenized_answer[:, :dimension]
                                        for i in range(questions_amount):
                                            current_questions_batch[i] = torch.nn.functional.pad(current_questions_batch[i], (0, dimension-current_questions_batch[i].shape[1]), value=0).to(device)
                                    else:
                                        answer = torch.nn.functional.pad(tokenized_answer, (0, dimension-tokenized_answer.shape[1]), value=0).to(device)
                                        for i in range(questions_amount):
                                            current_questions_batch[i] = current_questions_batch[i][:, :dimension]
                                else:
                                    answer = torch.nn.functional.pad(tokenized_answer, (0, size-tokenized_answer.shape[1]), value=0).to(device)
                                    for i in range(questions_amount):
                                        current_questions_batch[i] = torch.nn.functional.pad(current_questions_batch[i], (0, size-current_questions_batch[i].shape[1]), value=0).to(device)

                            # Make attention masks
                            for question in current_questions_batch:
                                current_questions_attention_masks_batch.append((question != 0).int().to(device))
                            current_answer_attention_mask = (answer != 0).int().to(device)
                    ...
                    for batch in train_loader:
                        ...
                        decoder_attention_mask = batch["decoder_attention_mask"].squeeze(0).to(device)
